...
ricianchan = ricianFading(10,N_taps) #initialize random rician channel
    ...
    for snr in tqdm(snr_values_dB):
        ...
        for _ in range(num_trials):
            ...
            if puncturing_pattern is not None:
                ...
                h_zp = np.concatenate((ricianchan, np.zeros(len(after_ifft) - N_taps)))  # Zero-padded channel impulse response (length Nc)
...
step_size = 0.5
snr_values_dB21= np.arange(0, 25, step_size)
